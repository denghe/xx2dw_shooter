...
struct Pumpkin : CircleObj {
		...
		while (true) {
			...
			if (numCross) {
				if (combineForce.IsZero()) {	// move by random angle
					...
					newPos += XY{ std::cos(r), std::sin(r) } * cSpeed/* + 0.5f*/;
				} else {	// move by v
					newPos += combineForce.MakeNormalize() * cSpeed/* + 0.5f*/;
				}
			}
	...
	void Init(Vec2<> const& pos) {
		radius = (cSize.x + (cSize.x % 2 == 0 ? 0 : 1)) / 2;
	...
	void Draw() const override {
		quad.TrySetFrame(gLooper.frames_pumpkin[(int32_t)frameIndex])
			...
			.SetPosition(gLooper.camera.ToGLPos(_sgcPos.As<float>()))
			.Draw();
	}
};
