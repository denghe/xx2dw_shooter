...
bool BitmapDC::setFont(std::string_view svFontName, int nSize, bool enableBold) {
...
SIZE BitmapDC::sizeWithText(std::wstring_view text, DWORD dwFmt, std::string_view fontName, int textSize, LONG nWidthLimit, LONG nHeightLimit, bool enableWrap, Overflow overflow) {
...
int BitmapDC::drawText(std::string_view text, SIZE& tSize, TextAlign eAlign, std::string_view fontName, int textSize, bool enableWrap, Overflow overflow) {
...
void BitmapDC::removeCustomFont() {
    HFONT hDefFont = (HFONT)GetStockObject(DEFAULT_GUI_FONT);
    if (hDefFont != _font) {
        DeleteObject(_font);
        _font = hDefFont;
...
xx::Data BitmapDC::getTextureDataForText(std::string_view text, const FontDefinition& textDefinition, TextAlign align, int& width, int& height) {
