...
namespace TMX {
	...
	struct Filler {
		...
		void TryFillImage(xx::Ref<Image>& out, pugi::xml_node const& c);
		...
		void TryFillObject(xx::Ref<Object>& o, pugi::xml_node const& c);
	...
	// example: <polyline points="-65.1667,63.1667 190.5,317.667"/>
	inline static void FillPointsTo(std::vector<Pointf>& out, std::string_view sv) {
		Pointf p{};
		std::string s;
		while (sv.size()) {
			auto i = sv.find_first_of(',');
			s = sv.substr(0, i);
			p.x = std::atof(s.c_str());
			sv = sv.substr(i + 1);
			i = sv.find_first_of(' ');
			if (i != sv.npos) {
				s = sv.substr(0, i);
	...
	inline static void FillPointsTo(std::vector<Pointf>& out, pugi::xml_attribute const& a) {
		FillPointsTo(out, a.as_string());
	}
	...
	void Filler::TryFillImage(xx::Ref<Image>& out, pugi::xml_node const& c) {
		out.Reset();
		if (c.empty()) return;
		std::string s;
		TryFill(s, c.attribute("source"));
		auto&& fp = rootPath + s;	// to fullpath
		auto&& iter = std::find_if(map.images.begin(), map.images.end(), [&](xx::Ref<Image> const& img) {
			return img->source == s;
			});
		if (iter == map.images.end()) {
			...
			if (autoLoadTextures) {
				img->texture = EngineBase1::Instance().LoadTexture(fp);
			}
	...
	void Filler::TryFillTileset(Tileset& ts, pugi::xml_node const& c) {
		...
		if (auto&& d = EngineBase1::Instance().LoadFileDataWithFullPath(fp); !d) {
	...
	void Filler::TryFillObject(xx::Ref<Object>& o, pugi::xml_node const& c) {
		...
		if (auto&& cText = c.child("text"); o && o->type == ObjectTypes::Text || !cText.empty()) {
			...
			if (auto&& attr = cPolygon.attribute("points"); !attr.empty()) FillPointsTo(a->points, attr.as_string());
			o = std::move(a);
		} else if (auto&& cPolygon = c.child("polyline"); o && o->type == ObjectTypes::Polygon || !cPolygon.empty()) {
			xx::Ref<Object_Polygon> a;
			if (o) {
				a = o.Cast<Object_Polygon>();
			} else {
	...
	void Filler::TryFillLayer(Layer_Object& L, pugi::xml_node const& c) {
		...
		for (auto&& cObject : c.children("object")) {
			...
			if (auto&& aTemplate = cObject.attribute("template"); !aTemplate.empty()) {
				...
				if (auto&& [d, fp] = EngineBase1::Instance().LoadFileData(fn); !d) {
	...
	void Filler::TryFillLayer(Layer_Group& pL, pugi::xml_node const& pC) {
		...
		for (auto&& c : pC.children()) {
			...
			if (name == "layer"sv) {
				auto&& L = xx::MakeRef<Layer_Tile>();
				TryFillLayer(*L, c);
				pL.layers.emplace_back(std::move(L));
			} else if (name == "imagelayer"sv) {
				auto&& L = xx::MakeRef<Layer_Image>();
				TryFillLayer(*L, c);
				pL.layers.emplace_back(std::move(L));
			} else if (name == "objectgroup"sv) {
				auto&& L = xx::MakeRef<Layer_Object>();
				TryFillLayer(*L, c);
				pL.layers.emplace_back(std::move(L));
			} else if (name == "group"sv) {
				auto&& L = xx::MakeRef<Layer_Group>();
				TryFillLayer(*L, c);
				pL.layers.emplace_back(std::move(L));
			} else {
		...
		, autoLoadTextures(autoLoadTextures)
	{
		...
		if (auto&& [d, fp] = EngineBase1::Instance().LoadFileData(tmxfn); !d) {
		...
		for (auto&& [ps, idx] : objProps) {
			auto& p = (*ps)[idx];
			p.value = objs[(uint32_t)std::get<int64_t>(p.value)];
		}
		objs.clear();
		objProps.clear();
	}
