...
inline GLShader LoadGLShader(GLenum type, std::initializer_list<std::string_view>&& codes_) {
    ...
    if (!r) {
		...
		xx::CoutN("glCompileShader failed: err msg = ", s);
		xx_assert(false);
    }
...
inline GLProgram LinkGLProgram(GLuint vs, GLuint fs) {
    ...
    if (!r) {
		...
		xx::CoutN("glLinkProgram failed: err msg = ", s);
		xx_assert(false);
    }
...
inline GLTexture LoadGLTexture(std::string_view const& buf, std::string_view const& fullPath) {
	...
	LabError:
		xx::CoutN("bad astc file header. fn = ", fullPath);
		xx_assert(false);
	}
	...
	xx::CoutN("unsupported texture type. fn = ", fullPath);
	xx_assert(false);
	return {};
}
